...
// Host bridge (window.omelette.writeFile) can land AFTER the first render,
// which leaves slots stuck in read-only mode until hovered. Nudge them once
// it appears so drops register and persist immediately.
(() => {
  let tries = 0;
  const nudge = () => {
    document.querySelectorAll('image-slot').forEach((el) => {
      try { el.dispatchEvent(new Event('pointerenter')); } catch (e) {}
    });
  };
  const tick = () => {
    if (window.omelette && window.omelette.writeFile) { nudge(); return; }
...
// ── IMAGE-SLOT LOCAL MIRROR ──────────────────────────────────────────────────────
// The host's sidecar write channel can refuse writes (it circuit-breaks
// after repeated failures), which silently loses dropped images on reload.
// Mirror every sidecar write into IndexedDB and serve it back on read, so
// drag-and-drop always survives a reload in this browser even when the
// project-file write fails. The real sidecar is still written when it works;
// the mirror only fills gaps.
(() => {
  const FILE = '.image-slots.state.json';
  const KEY = 'image-slots' + location.pathname.replace(/[^/]*$/, '');
  const DB = 'image-slot-mirror';
  ...
  const readMirror = () => withStore('readonly', (s) => s.get(KEY));
  const writeMirror = (v) => withStore('readwrite', (s) => s.put(v, KEY));

  // Read side: merge the mirror over whatever the sidecar fetch returns.
  const realFetch = window.fetch.bind(window);
  window.fetch = function (input, init) {
    const url = typeof input === 'string' ? input : (input && input.url) || '';
    if (!url.includes(FILE)) return realFetch(input, init);
    return Promise.all([
      realFetch(input, init).then((r) => (r.ok ? r.json() : null)).catch(() => null),
      readMirror(),
    ]).then(([disk, mirror]) => {
      const merged = Object.assign({}, disk || {}, mirror || {});
  ...
  // Write side: always mirror; forward to the host when it will take it.
  function install() {
    const om = window.omelette || (window.omelette = {});
    if (om.__slotMirror) return;
    const real = om.writeFile;
    om.__slotMirror = true;
    om.writeFile = function (path, data) {
      if (String(path).includes(FILE)) {
        let parsed = null;
        try { parsed = JSON.parse(data); } catch (e) {}
        if (parsed) writeMirror(parsed);
        if (!real) return Promise.resolve(true);
        try { return Promise.resolve(real.call(om, path, data)).catch(() => true); }
        catch (e) { return Promise.resolve(true); }
      }
      return real ? real.call(om, path, data) : Promise.resolve(false);
    };
  }
  install();
  let n = 0;
  const watch = setInterval(() => {
    if (window.omelette && !window.omelette.__slotMirror) install();
    if (++n > 40) clearInterval(watch);
  }, 250);
})();
